...
static void update_data_blocks(BMesh *bm, CustomData *olddata, CustomData *data)
{
  ...
  for (int i = 0; i < olddata->totlayer; i++) {
    if (olddata->layers[i].flag & CD_FLAG_ELEM_NOCOPY) {
      ...
      nocopy_layers.append(olddata->layers + i);
    }
  }
  ...
  for (CustomDataLayer *layer : nocopy_layers) {
...
void BM_data_layers_ensure(BMesh *bm, CustomData *data, BMCustomLayerReq *layers, int totlayer)
{
  ...
  if (old.layers) {
    old.layers = static_cast<CustomDataLayer *>(MEM_dupallocN(old.layers));
  }
  ...
  for (int i = 0; i < totlayer; i++) {
    ...
    if (req->name) {
      idx = CustomData_get_named_layer_index(data, eCustomDataType(req->type), req->name);
    }
    else {
      idx = CustomData_get_layer_index(data, eCustomDataType(req->type));
    }
    ...
    if (idx < 0) {
      ...
      if (req->name) {
        CustomData_add_layer_named(data, eCustomDataType(req->type), CD_SET_DEFAULT, 0, req->name);
        idx = CustomData_get_named_layer_index(data, eCustomDataType(req->type), req->name);
      }
      else {
        CustomData_add_layer(data, eCustomDataType(req->type), CD_SET_DEFAULT, 0);
        idx = CustomData_get_layer_index(data, eCustomDataType(req->type));
      }
